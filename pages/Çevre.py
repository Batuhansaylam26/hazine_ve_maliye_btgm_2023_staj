...
layout = html.Div(
    [
        ...
        html.Div([dcc.Graph(id="pollution")]),
        html.Div([dcc.Graph(id="thsp")]),
    ]
)


@callback(Output("energy", "figure"), Input("Regions", "value"))
def renderplot(reg):
    ...
    fig1.add_trace(
        go.Scatter(
            x=data["country"],
            y=data["CO2 emission estimates (million tons/tons per capita)"],
            mode="lines",
            name="CO2 emisyon (milyon ton)",
        ),
        secondary_y=True,
    )
    fig1.add_trace(
        go.Bar(
            x=data["country"],
            y=data["Energy production, primary (Petajoules)"],
            name="Enerji Üretimi (Petajoules)",
        )
    )
    fig1.update_layout(
        xaxis=dict(
        ...
        yaxis=dict(title="Enerji Üretimi (Petajoules)"),
        yaxis2=dict(title="CO2 emisyon(milyon ton)"),
    )
    return fig1


@callback(Output("pollution", "figure"), Input("Regions", "value"))
def renderplot2(reg):
    data = data11[data11["Region"] == str(reg)]
    fig = px.bar(
        data,
        x="country",
        y="Pollution index",
        hover_data=["Pollution index", "Climate index"],
        color="Climate index",
        labels={"Pollution index": "Kirlilik indeksi", "country": "Ülkeler"},
        height=400,
    )
    return fig


@callback(Output("thsp", "figure"), Input("Regions", "value"))
def renderplot3(reg):
    data = data11[data11["Region"] == str(reg)]
    fig = go.Figure()
    fig.add_trace(
