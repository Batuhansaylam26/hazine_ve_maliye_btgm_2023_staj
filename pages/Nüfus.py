...
for i in data2["Region"].unique():
    ...
    dicti[i] = (
        data11[data11["gender"] == "female"].values[0, 1]
        / data11[data11["gender"] == "male"].values[0, 1]
    )
...
fig1.add_trace(
    go.Scatter(
        ...
        y=dicti["ratio"],
        mode="lines",
        name="Kadın nüfusun erkek nüfusa oranı",
    ),
    secondary_y=True,
)
...
fig1.add_trace(
    go.Bar(
        x=data2[data2["gender"] == "male"]["Region"],
        y=data2[data2["gender"] == "male"]["count"],
        name="Erkek nüfus",
    ),
    secondary_y=False,
)
...
fig1.add_trace(
    go.Bar(
        x=data2[data2["gender"] == "female"]["Region"],
        y=data2[data2["gender"] == "female"]["count"],
        name="Erkek nüfus",
    )
)
...
# Edit the layout
fig2.update_layout(
    title="Bölgelere göre eğitim seviyeleri",
...
fig3.update_layout(
    ...
    yaxis=dict(title="Nüfus (Bin) (2017)"),
    yaxis2=dict(
...
@callback(Output("pop-traffic", "figure"), [Input("memory", "value")])
def scline(reg):
    data1 = pandas.DataFrame(columns=data.columns.tolist())
    for i in reg:
        data1 = data1._append(data[data["Region"] == i])
    fig = px.scatter(
        data1,
        x="Population density (per km2, 2017)",
        y="Traffic commute time index",
        marginal_x="box",
        marginal_y="violin",
        color="Region",
    )
