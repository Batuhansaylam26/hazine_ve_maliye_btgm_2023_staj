...
Q3, Q1 = np.percentile(
    data["GDP: Gross domestic product (million current US$)"].tolist(), [75, 25]
)
IQR = Q3 - Q1
lower = Q1 - 1.5 * IQR
upper = Q3 + 1.5 * IQR
...
upper_array = np.where(
    data["GDP: Gross domestic product (million current US$)"] >= upper
)[0]
lower_array = np.where(
    data["GDP: Gross domestic product (million current US$)"] <= lower
)[0]
...
xrange = np.linspace(
    X["Population in thousands (2017)"].min(),
    X["Population in thousands (2017)"].max(),
    100,
)
...
fig10.add_trace(
    go.Scatter(
        ...
        mode="markers",
        marker=dict(
            color=data["GDP: Gross domestic product (million current US$)"],
            colorscale="viridis",
            showscale=True,
        ),
    )
...
fig10.add_trace(
    go.Scatter(x=xrange, y=pred, name="pe m", line=dict(color="royalblue", width=4))
)


# Edit the layoutGDP: Gross domestic product (million current US$)"],
fig10.update_layout(title="GDP vs Nüfuz", xaxis_title="Nüfuz", yaxis_title="GDP")
...
fig2 = ff.create_scatterplotmatrix(
    df2, diag="histogram", index="Region", colormap_type="seq", height=1000, width=1800
)
fig2.update_layout(
    height=2000, margin={"r": 0, "t": 0, "l": 0, "b": 0}, paper_bgcolor="rgba(0,0,0,0)"
)

layout = html.Div(
    [
        html.Div(["Regression Plot:"]),
        html.Div(
            [dcc.Graph(figure=fig10)], style={"width": "48%", "display": "inline-block"}
        ),
        html.Div(
            [html.P(str(ressum))],
...
@callback(Output("unemp", "figure"), Input("Regions", "value"))
def renderplot(reg):
    ...
    fig.add_trace(
        go.Scatter(
            x=data["country"],
            y=data["Unemployment (% of labour force)"],
            mode="lines",
            name="İşsizlik (% İş gücü)",
        ),
        secondary_y=True,
    )
    fig.add_trace(
