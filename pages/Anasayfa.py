...
fig.update_layout(
    height=1000, margin={"r": 0, "t": 0, "l": 0, "b": 0}, paper_bgcolor="rgba(0,0,0,0)"
)
...
layout = html.Div(
    children=[
        html.Div(
            [dcc.Graph(id="mapgraph", figure=fig)],
        ...
        html.Div(
            [
                ...
                dcc.Dropdown(
                    data.select_dtypes(include="number").columns.unique(),
                    id="x",
                    value=data.select_dtypes(include="number").columns.unique()[0],
                    style={"background-color": "#D62728", "margin": "0 auto"},
                ),
            ],
        ...
        html.Div(
            [
                html.Div([dcc.Graph(id="scatters", figure=fig2)]),
                html.Div([dcc.Graph(figure=fig15)]),
            ],
...
@callback(Output("mapgraph", "figure"), Input("x", "value"), Input("scope", "value"))
def map_plot(x, sc):
    fig = px.choropleth(
        data,
        locations="iso3",
