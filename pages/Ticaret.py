...
fig.append_trace(go.Bar(
    ...
    marker_color='indianred',orientation='h',name="Toplam savunma harcaması (% GDP)"
),1,1)
fig.add_trace(
    go.Bar(
        ...
        marker_color='rgba(246, 78, 139, 0.6)',orientation='h',name="Toplam sağlık harcaması (% GDP)"
    )
)
...
fig.append_trace(go.Line(
    y=data["Region"].unique(),
    x=data.groupby("Region")["Tax revenue (% of GDP)"].sum(),name="Vergi geliri (% GDP)"
),1,2)
fig.update_layout(
    barmode="stack",
    yaxis=dict(
...
fig1.add_trace(go.Bar(
    x=data["Economy: Agriculture (% of GVA)"], y=data["Region"].unique(),
    orientation='h',
    marker=dict(
...
fig1.add_trace(go.Bar(
    x=data["Economy: Industry (% of GVA)"], y=data["Region"].unique(),
    orientation='h',
    marker=dict(
        line=dict(color='rgb(248, 248, 249)')
...
fig1.add_trace(go.Bar(
    x=data["Economy: Services and other activity (% of GVA)"], y=data["Region"].unique(),
    orientation='h',
    marker=dict(
        line=dict(color='rgb(248, 248, 249)')
...
fig3 = px.treemap(data, path=[px.Constant("world"),"Region","country"], values="International trade: Exports (million US$)",
                  ...
                  color_continuous_midpoint=np.average(data["International trade: Imports (million US$)"]))
